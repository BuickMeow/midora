...
| 非零范围 Buffering 恢复 | 非零 `startTick` 的 canonical range、锁存失败 tick `F`、canonical 中的 Project 源拍号图 | 使用源拍号图计算 Project absolute natural bar；范围起点的恢复拍号仍只作为 range event | `CanonicalConductor.TimeSignatures` 保持范围恢复语义；`SourceTimeSignatureMap` 仅提供确定性小节上下文，不输出为重复 MIDI Meta | 规划、tick/frame 换算或 backend 恢复请求异常统一进入 `PlaybackState.Error`，释放任务与编辑锁，只报告一次错误 |

### 2.1 Canonical Conductor 设计决定

- `CanonicalConductor.TimeSignatures` 继续表示当前编译范围内的 Time Signature 事件；非零范围从范围起点恢复当前有效拍号，供 MIDI 等正式消费者使用。
- `CanonicalConductor.SourceTimeSignatureMap` 保存当前 canonical 结果所依赖的 Project 源拍号图，保留原始 tick 和稳定 ID；Buffering 恢复使用该集合构造第 4 章统一的 `ProjectTimeSignatureMap`，不得把范围起点恢复事件误当成 Project tick 0 事件。
- 源拍号图进入 canonical content fingerprint，因此 Full / Incremental、缓存与重放对同一输入保持一致；它不进入 `.midora`、MIDI 事件流或音频 Worker IPC。
...
- 本修复只为 canonical Conductor 增加源拍号图上下文；不改变范围内 Time Signature 输出、tick→sample、自然小节恢复算法、Stop Cursor Behavior、SoundFont 引用或 `.midora` 格式。
...
7. 非零 tick 冷启动的 canonical range 必须同时保留范围起点恢复拍号和 Project 源拍号图；Buffering 恢复按源图得到与整曲编译相同的自然小节边界。
8. Buffering 恢复规划中的任何异常必须在一次 `Update` 内完成 backend Stop、sample cache 失效、任务/编辑锁清理并进入 Error；WPF timer 不得再次抛出同一异常或形成模态弹窗循环。
