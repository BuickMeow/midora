...
MIDI 与音频必须调用同一公共合法化算法；相同候选文件名、扩展名预算和冲突上下文必须得到相同文件名部分。精确替换字符、Unicode 规范化形式、文件名部分最大长度和冲突后缀格式必须由后续已接受决定统一固定；在固定前不得隐藏在实现默认值中。

若安全合法化后仍无法形成唯一、合法、可表示的完整目标列表，预检查必须阻止任务并产生文件系统诊断。
### 14.17.5 路径过长
文件名部分允许按照第 14.17.4 节的固定公共算法为扩展名和冲突后缀保留空间。该合法化不得根据用户选择的父目录深度临时改变同一候选名称的结果。
