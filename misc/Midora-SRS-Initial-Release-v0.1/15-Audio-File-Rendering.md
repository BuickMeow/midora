...
文件渲染使用第 13.30 节为初版最终选定的音频后端拓扑。采用内部音频子进程时：
```text
子进程接收冻结的 canonical compiled result、文件采样率、输出链设置、SF2 资源信息和已授权临时目标。
子进程不得读取或解释 Project 源对象。
文件专用 OutputDevice 可以位于子进程内，但文件事务授权、最终发布结果和用户任务状态仍由主应用协调。
IPC block / buffer 大小不得改变采样位置、事件顺序、最终 frame 数或 RIFF/WAVE 内容语义。
```
---
## 15.8 音频输出链
### 15.8.1 统一链路
...
### 15.8.3 Limiter
音频文件渲染必须应用与实时播放相同语义的 Limiter，并使用正式生效的 Playback Limiter 参数语义。
初版：
```text
...
如果 Limiter 存在 look-ahead 或内部延迟：
```text
离线渲染必须保持与播放一致的时间语义。
不得让整首音乐整体后移。
...
```
### 15.9.2 有限样本
有限但超出 `[-1, 1]` 的 float 样本允许写入 RIFF/WAVE IEEE float32 数据。
初版不：
```text
检测 clipping
提示 limiter activity
...
具体 Unicode 规范化形式和最大长度由实现层决定，但必须稳定一致，并为扩展名及冲突序号保留空间。
### 15.10.5 扩展名
整曲路径缺少扩展名时自动追加：
```text
.wav
...
输出目录
```
生成并冻结完整目标列表，并在开始前向用户预览或至少完成冲突检查。
