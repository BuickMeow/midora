...
19. Pure MIDI 正式模型固定为 `MIDI Channel Root → Pure MIDI Track → Midi Segment → Direct MIDI Note/Event`。Root 是持久 Unit 身份；子 Segment End 只关闭本 Segment Note，Root 活动连通区间结束才做 CC120/最终 Reset。分配顺序为 Fixed Roots→非空 Auto Roots→剩余 Logical groups，combined peak 不得超过 256。
...
22. Arrangement 正式结构固定为 Conductor 第一行，以及可混排的 Event Instrument / MIDI Channel Root parents；Logical / Pure MIDI Tracks 必须分别是对应 parent 的唯一 children。删除 Project Panel、可见 Event Instrument Library Workspace/Folder、独立全局 Track/Root 顺序和 Unbound Logical Track。完整规则见 SRS 第 24 章与 INV-058～064。
23. parent 普通 Copy/Paste/Duplicate 深拷贝完整 subtree 并重映射所有新稳定 ID；Root 副本强制 Auto。Event Instrument 另有 `Duplicate Instrument Only`。删除 non-empty parent 必须确认并原子级联 children，不得生成孤儿或 Unbound Track。
24. Event Instrument / Root parent 与 child Track 的 Mute/Solo 相互独立且只属运行期；父 Solo 存在时忽略 child Solo，否则 child Solo 使用全局候选，parent/child Mute 始终过滤。Logical/Direct Note 跨类型剪贴板只转换共同字段；Direct NoteOff velocity 在 Direct 数据链和 SMF 中必须保留。
...
- Arrangement 的当前正式导航/层级与高性能概览额外服从 SRS 第 24 章：无 Project Panel，Conductor 固定第一行，Event Instrument/Root 混排父节点与其 child Tracks 两级呈现；Pure MIDI event preview 必须位于 Note 上层且为 50% 透明度，Conductor/Segment 极端内容使用可视瓦片缓存和局部失效。
