...
Logical Track 位于 Logical Tracks 集合中
...
Logical Track 可以处于未指定 Event Instrument 状态
```
Logical Track 负责承载：
```text
Segment
...
创建 Logical Track 时：
```text
如果当前有选中的 Event Instrument，可默认绑定该 Event Instrument，并默认使用其名称作为 Track 名称
如果没有选中 Event Instrument，可创建未指定 Event Instrument 的空 Track
```
Track 名称不是内部身份。
### 11.2.3 颜色
...
```
没有选中 Event Instrument 时：
```text
创建未指定 Event Instrument 的空 Logical Track
```
### 11.3.2 改绑
...
取消绑定后：
```text
Track 进入未指定 Event Instrument 状态
Segment、Logical Note、Logical Parameter Lane 数据保留
该 Track 不参与编译、播放、实时渲染或音频文件渲染结果
该 Track 不产生 Event Instrument Instance
该 Track 不占用 Channel Unit
```
如果未指定 Event Instrument 的 Track 包含内容：
```text
全项目诊断中列为信息
不算警告
不导致编译失败
...
新 Track 获得新的稳定 ID
新 Track 绑定同一个 Event Instrument 定义
...
```
如果目标 Track 未指定 Event Instrument：
```text
允许移动
内容保留
目标 Track 不参与编译 / 播放 / 渲染
```
...
目标 Track 未指定 Event Instrument：数据保留但不参与编译
```
---
...
但不自动创建 Lane 数据
```
### 11.14.5 未指定 Event Instrument
Logical Track 未指定 Event Instrument 时：
```text
已有参数 Lane 数据保留
显示为断裂 / 不适用
不允许新增参数 Lane
...
---
## 11.23 诊断入口
### 11.23.1 未指定 Event Instrument 的非空 Track
未指定 Event Instrument 的非空 Logical Track：
```text
诊断等级：信息
不算警告
不导致失败
