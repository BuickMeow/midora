...
- Runtime ownership: Selection state bookmarks, clipboard payloads, `A` shortcut routing, transient dialog validation, compiled Batch Edit delegates and viewport state are session-only. Note/Event presets are external local JSON resources under `%LOCALAPPDATA%\Midora\Presets`; they are deliberately not Project data or Application Preferences.
