...
- Mapping Function Expression：唯一名称、ABI v3 单行表达式精确文本和自动推导的 Context 依赖集合可原子更新；函数/Step ID 不变，Apply 将旧 ABI 升级为 v3。表达式执行有效 Unicode 与 8,192 scalar 上限，验证错误可保存且参与使用时失败。删除被任何 Step 持有 Function ID 的函数前必须确认；删除保留断裂 ID，Undo 恢复原函数对象、表达式、ABI、依赖、索引与引用。
- Mapping Chain / Step：事件共享 Mapping 与 Logical Parameter Mapping 均按稳定 Chain ID 定位；Chain/Step Enable、Step 全配置更新、Step 手动排序/删除和整链删除进入 History。Step 更新只阻止未定义枚举、NaN/Infinity 与空稳定引用 ID，允许倒置范围、空范围和断裂/未完成引用进入 Project，再由活动 canonical 路径诊断；禁用 Chain/Step 中的错误配置不诊断。Q-NUI-008 的后续决定已取代空链 sentinel：Note Number/Velocity owner 强制保留；非 Note `SubVoiceEventMapping` 与 `LogicalParameterMapping` 整链删除时物理移除 owner，Undo 恢复同一 owner、Chain、Target Settings、Step、引用和顺序。非空链删除仍要求显式确认；事件点保留并以原始值直通。Compiler 测试逐项覆盖全部内置 Source/Operation、Remap 输入越界和 DivideByZero policy。
...
- Project SoundFont（不分配 ID 分支）：External 选择/替换/重新绑定先完成路径、完整 hash/size、正式 BASSMIDI loadability 与提交前内容复核，再把 source reference 作为 History 命令提交；取消选择同步清空引用。加载失败、验证期间文件变化和锁冲突均零变更，重复选择/清空无操作。运行时会话监听 History 的 CompilationChanged：任何 source reference 变化立即清空临时 effective path 并要求按新引用重新验证，故 Undo/Redo 不会消费旧验证结果。Embedded 导入仍因 resource ID 分配与跨 History 资源租约所有权等待 Q-NUI-005。
...
- 本切片提供通用属性命令及上述不分配稳定 ID 的具体领域命令。创建 Event Instrument/Folder/Track/Segment/Note/Lane/Point、复制 Event Instrument/Track/Segment、Segment Split 和创建 Conductor/End Marker 事件等 ID 分配命令的 Undo/Modified 语义由 Q-NUI-005 决定后接入；Q-NUI-003 决定前不扩展 MIDI Export Settings v2；Q-NUI-009 决定前不实现需要全 Project Lane 数据迁移的 Logical Parameter Definition 结构编辑。
