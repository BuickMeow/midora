...
17. Midora 初版定位为免费、开源、非商业软件；Midora 自有源代码固定使用未经自定义修改的标准 MIT License，项目自身非商业不得转化为限制下游商业使用的附加条款。该许可证不覆盖 BASS/BASSMIDI/BASSWASAPI，也不自动证明满足其免费使用条件；正式分发前必须按实际主体、收入、平台、分发方式和届时有效条款完成核验并提供第三方 notices。正式 `LICENSE` 的版权持有人尚待产品所有者确认，不得从 Git 账号、仓库 URL 或提交者名称推断。
