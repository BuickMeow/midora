...
第一阶段允许现有 `MidoraCompiler` 暂时继续消费 compiler-owned `MidoraProject` mirror，因此 gate 外仍可能发生与受影响 owner 内容量线性相关的对象物化；这只能作为迁移步骤，不能重新扩大 UI 临界区。当前 Pure MIDI Content Pack 页可以直接共享，但 editable overlay 在正式 collection insertion order 尚未拥有 immutable order root 前仍需独立捕获；现有范围查询 snapshot 不能代替该正式顺序。该过渡路径只算部分完成，不能宣称已经满足超大 Pure MIDI overlay 的锁内性能门。后续 compiler 入口应收敛为直接消费版本化 immutable compilation graph，并为 Pure MIDI 补齐正式 order root，使分页值在 Validation、Full/Incremental Compile 和 canonical fingerprint 中按确定顺序读取，不再重建等量领域对象。无论处于哪一迁移阶段，同一源 revision 的 Full/Incremental 输出、诊断来源、同 tick 顺序和 fingerprint 必须完全一致，缓存或取消时机不得改变语义。

所有 Piano、Velocity 与 Event/Parameter Lane 使用 world-anchored、离散量化 LOD；连续 pan/zoom 直接仿射显示最近完整 LOD，后台仅补当前量化 LOD。基础内容、选择和手势预览是独立图层。Move/Copy 可以平移现有选择层；左右边界 Resize 必须使用 latest-only 的有界像素瓦片预览，严禁按所选对象数量逐帧构造 WPF Geometry。Arrangement 继续严格保留唯一 zoom-independent coarse fallback 与分级 detail 两层；detail 只局部替代 coarse。所有 range fingerprint 由相交页摘要组合，`OnRender`、hover 与 hit test 不允许读取文件、Brotli 解压、SHA-256、全量枚举或大型排序；hit test 使用专用有界邻近索引，不复用渲染范围查询。
