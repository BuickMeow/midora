...
15. MIDI 导出和音频文件渲染必须共用确定性的 Windows 安全文件名合法化与冲突检测服务：NFC、SRS 14.17.4 固定不安全字符表连续段替换为 `_`、设备保留名前缀 `_`、255 UTF-16 code unit、text-element 安全截断、NFC + OrdinalIgnoreCase 冲突键、稳定 ` (n)` 后缀。任务开始前预览并冻结全部最终路径；合法化不修改 Project 源名称、不进入 Undo / Redo，已有目标不参与后缀分配且仍须明确覆盖授权。最终文件/Track Name/Readme 模板尚未确认时不得隐藏选择。
