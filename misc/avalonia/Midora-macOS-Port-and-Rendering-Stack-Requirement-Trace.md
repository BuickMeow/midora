...
## Slice T：Skia GPU 批渲染最小验证（2026-09-21）

独立 spike（仓库外，`Avalonia 12.1.2 + Avalonia.Skia + SkiaSharp 3.119.4`，macOS arm64，窗口 1600×900，
每帧平移视口，240 帧；绘制成本在 `ICustomDrawOperation` 内计时并在绘制后 `SKCanvas.Flush()`，含 GPU 提交）：

| 路径 | 音符数 | avg | p50 | p99 | 每音符成本 |
| --- | --- | --- | --- | --- | --- |
| `SKCanvas.DrawVertices`（每批 16k 四边形，共 63 批） | 1,000,000 | **6.43 ms** | 6.30 ms | 7.59 ms | ≈ 6.4 ns |
| 同上 | 4,000,000 | 23.19 ms | 22.00 ms | 62.82 ms | ≈ 5.8 ns |
| 逐图元 `DrawRect`（现路径近似） | 50,000 | 5.18 ms | 5.29 ms | 5.88 ms | ≈ 104 ns |
| 逐图元 `DrawRect` | 200,000 | 17.82 ms | 17.66 ms | 20.73 ms | ≈ 89 ns |

结论：

- `DrawVertices` 的**每音符成本比逐图元低约 15 倍**（6 ns vs 90–100 ns）；
  1M 音符仅 6.4 ms/帧，仍有大量余量（帧间隔稳定 16.1 ms / 62 FPS，vsync 限制）。
- 无剔除时 4M 音符已超出 16.7 ms 预算（23 ms）→ 需要**视口子集提交**（复用现有
  `TimelineIntervalIndex`），使提交量由屏幕可见量决定而非内容总量。
- 像素校验：1M 音符经 `RenderTargetBitmap` 渲染后非背景像素 125,086，确认顶点批真实出图。
- spike 保真度：50k 逐图元的 5.18 ms 与 Midora 内实测（~50k ≈ 4.3 ms）同量级，说明测量可信。

**决定**：性能显著优于现路径，按产品所有者要求正式接入（混合架构：Skia 继续负责 UI 与交互，
...
## Slice W 修订：2× 阶梯与"目标单元宽度"选级

产品所有者评审结论：4× 步进的可用档位太少（实测只出现 1/16/256/1024，且最宽缩放时单元只有 6px，
离 16px 目标很远），改为 **2× 步进**（16、32、64、128、…）。

选级规则（`TimelinePianoRollLod`，纯策略、可单测）：

- `target = 16px`；取**不超过** target 的最大阶梯值：`1, 16, 32, 64, 128, 256, …`；
- 音符平均宽度 ≥ 1px → `1`（**逐音符精确**，不合并）；≤1px → 从 16 起，每级 ×2；
- 单元宽度保持在 `(8, 16]px`（半个到一个 target），且**无上限**（仅受 int 溢出保护），
  极端拉远时自然合成大块；
- 合并始终是**保守并集**：单元包络 = 组内音符的 `[首个起点, 最大终点]`，绝不丢音符、不设四边形数量上限；
- 选中音符**不做特殊处理**（合并档位下不画精确覆盖层）。

实测（20 万音符，`MIDORA_TRACK_BENCH=100`）：

| span | 可见音符 | 档位 | p50 |
| --- | --- | --- | --- |
| 300 | 300 | 1（精确） | 1.10 ms |
| 2,000 | 2,000 | 16 | 0.89 ms |
| 6,000 | 6,000 | 64 | 1.05 ms |
| 20,000 | 20,000 | 128 | 1.39 ms |
| 60,000 | 60,000 | 512 | 2.72 ms |
| 200,000 | 200,000 | 2048 | 4.78 ms |

同进程 A/B（span 200000，档位 2048 vs 逐音符）：**p50 4.07 ms vs 170.13 ms（42×）**。
