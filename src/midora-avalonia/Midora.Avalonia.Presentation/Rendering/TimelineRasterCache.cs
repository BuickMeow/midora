...
public sealed class TimelineRasterBuffer
{
    ...
    internal WriteableBitmap CreateFrozenBitmap() =>
        PixelBufferBitmap.Create(Pixels, Width, Height, 96, 96);
}
...
public static class TimelineRasterPlacement
{
        ...
        List<Rect> destination)
    {
        ...
        destination.Clear();
        if (!double.IsFinite(visibleBounds.Left)
    ...
    public static Rect GetSegmentPreviewTileDestination(
        ...
        DpiScale dpi)
    {
        if (double.IsFinite(fullSegmentBounds.Left)
...
internal sealed class TimelineRasterCache
{
    ...
    public bool TryGet(TimelineRasterCacheKey key, out WriteableBitmap? bitmap)
    {
        lock (_gate)
        {
