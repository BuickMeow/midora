...
public sealed record GenerationHelpField(string Label, string Text);

public sealed record GenerationHelpExample(
    string Title,
    string Description,
    IReadOnlyList<GenerationHelpField> Fields);

/// <summary>
/// Avalonia port of <c>Midora.Desktop.TimelineGenerationHelpDialog</c>. <paramref name="notes"/>
/// selects the Note or Event variant, exactly like the WPF constructor. The WPF
/// <c>PreviewKeyDown</c> Escape handler is covered by the footer Close button's IsCancel.
/// </summary>
public partial class TimelineGenerationHelpDialog : Window
{
    public TimelineGenerationHelpDialog()
        : this(false)
    {
    }
    ...
    public IReadOnlyList<GenerationHelpField> Variables { get; }

    public IReadOnlyList<GenerationHelpExample> Examples { get; }
