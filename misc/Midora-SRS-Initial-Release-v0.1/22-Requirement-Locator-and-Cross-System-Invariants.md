...
| INV-063 | Pure MIDI Segment 概览的 event 线固定绘制在 Note 上层、透明度 50%、最小宽度 1 device pixel，并按正式值域归一化高度。Arrangement Segment 概览最高精度使用 `96 pixels / quarter note` 固定基准、256-pixel tile、64-pixel 高度；较低精度只允许固定半八度 `1 / 2^(n/2)` LOD，精确 viewport zoom 不得直接形成 cache identity。显示必须选择不向下采样 source pixel 的固定层，全部 tile 从同一 device-pixel-snapped 完整 Segment 变换派生。最多两个后台 worker 为每个 Segment 预热完整且不超过 4 tile 的固定 LOD。缓存不得成为命中、编译或导出语义来源。 |
