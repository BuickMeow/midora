...
public partial class ColorPickerDialog : Window
{
    ...
    private void RenderColorWheel()
    {
        ...
        WritePixels(ColorWheelImage, WheelSize, WheelSize, pixels);
    }
    ...
    private void RenderBrightnessBar()
    {
        ...
        WritePixels(BrightnessImage, BrightnessWidth, BrightnessHeight, pixels);
    }

    private static void WritePixels(Image target, int width, int height, byte[] pixels)
    {
    ...
    {
        if (!e.GetCurrentPoint(WheelCanvas).Properties.IsLeftButtonPressed) return;
        _draggingWheel = true;
        e.Pointer.Capture(WheelCanvas);
        UpdateHueSaturation(e.GetPosition(WheelCanvas));
    }
    ...
    {
        if (_draggingWheel) UpdateHueSaturation(e.GetPosition(WheelCanvas));
    }

    private void OnBrightnessPointerPressed(object? sender, PointerPressedEventArgs e)
    {
        if (!e.GetCurrentPoint(BrightnessCanvas).Properties.IsLeftButtonPressed) return;
        _draggingBrightness = true;
        e.Pointer.Capture(BrightnessCanvas);
        UpdateBrightness(e.GetPosition(BrightnessCanvas));
    }

    private void OnBrightnessPointerReleased(object? sender, PointerReleasedEventArgs e)
    {
    ...
    {
        if (_draggingBrightness) UpdateBrightness(e.GetPosition(BrightnessCanvas));
    }
    ...
    {
        PreviewFill.Fill = new SolidColorBrush(Color.FromRgb(_red, _green, _blue));
        double radius = (WheelSize - 2) / 2d;
        double center = WheelSize / 2d;
