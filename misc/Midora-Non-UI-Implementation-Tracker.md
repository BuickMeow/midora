...
| NUI-02 | 完整领域源模型与编辑事务 | History/Modified 与四批非 ID 分配对象命令完成；ID 分配命令等待 Q-NUI-005 | §3～11 | Track/Instrument/Folder/Damaged/Segment、Conductor、Project Settings、Note/Lane/Point、Instrument Lifecycle 与 SubVoice 基础结构已覆盖；继续 Template Event/Curve/Mapping/Envelope 属性矩阵 |
...
- 四批不分配稳定 ID 的结构/设置/音乐内容编辑命令已接入统一 History；Template Event/Curve、Initial State、Logical Parameter Definition/Mapping、Mapping Function 与 Envelope 命令矩阵仍需继续扩充，创建/复制/分割类命令等待 Q-NUI-005。
