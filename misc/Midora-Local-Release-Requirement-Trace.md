...
- dirty worktree 允许用于本地预览，但文件名和 `release-manifest.json` 必须明确记录 `dirty`，不得冒充可复现正式发布。
...
- BASS baseline 校验、locked restore、任一 publish、必需文件检查、ZIP 单根目录检查或 SHA-256 生成失败时脚本失败。
