...
```
相对路径以 `.midora` 所在目录为基准，文件中使用 `/`，只允许以下两种形状：
```text
<fileName>.sf2
soundfonts/<fileName>.sf2
```
`soundfonts` 目录名、文件名和扩展名保存用户选择时解析到的实际大小写与原 Unicode scalar 序列，不执行 normalization。解析时逐路径分量优先使用 ordinal 精确匹配；精确匹配不存在时，只允许唯一的 ordinal-ignore-case 候选并产生 Warning；存在多个大小写近似候选时判定为歧义，SoundFont 不可用。Project 根目录与 `soundfonts/` 中的同名文件是两个不同相对路径，不构成冲突。
---
## 6.5 SF2 选择、取消选择与替换
### 6.5.1 创建项目时选择 SF2
...
哈希算法固定为完整原始文件字节的 SHA-256，保存为 64 个小写十六进制字符；计算必须流式进行，不得要求把整个 SF2 读入单个托管数组。

外部 SF2 在用户选择、替换、重新绑定或明确“接受当前文件”时，必须先完成完整 hash 与文件大小计算，再原子提交 Project 编辑。打开 Project 后异步执行完整验证；验证未完成前播放、预览和音频渲染保持不可用。首次音频任务以及文件监控/元数据表明内容可能变化后，必须复核验证缓存；缓存键至少包括解析后的绝对路径、可用时的文件身份、文件大小和最后修改时间，文件监控只能作为失效提示，不能替代首次完整 hash。

普通保存和保存副本不得因被动发现外部文件变化而更新已保存 hash。内嵌 SF2 在导入和写入 package 时流式计算 SHA-256；`soundfont-settings.json`、manifest 与实际未压缩资源字节的 hash 必须一致。
### 6.7.2 哈希不同时的行为
